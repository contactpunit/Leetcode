...
def mergeArrays(nums1, nums2):
    ...
    while first < m and second < n:
        if nums1[first][0] == nums2[second][0]:
            final.append([nums1[first][0], nums1[first][1] + nums2[second][1]])
            first += 1
            second += 1
        elif nums1[first][0] > nums2[second][0]:
            while first < m and second < n and nums2[second][0] < nums1[first][0]:
                final.append([nums2[second][0], nums2[second][1]])
                second += 1
        elif nums1[first][0] < nums2[second][0]:
            while first < m and second < n and nums1[first][0] < nums2[second][0]:
                final.append([nums1[first][0], nums1[first][1]])
                first += 1
    if first < m:
        final.extend(nums1[first:])
    elif second < n:
        final.extend(nums2[second:])
    return final
